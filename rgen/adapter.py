...
class Adapter:
    ...
    def _adapt_from_pattern(self, profile: ProjectProfile, pattern: dict) -> dict[str, str]:
        ...
        result[".github/routing-map.json"] = json.dumps(
            {"_base_autoloaded": {"note": "auto-generated by AgentPilot Orchestrator"},
             **adapted_map},
    ...
    def _adapt_from_scratch(self, profile: ProjectProfile) -> dict[str, str]:
        ...
        result: dict[str, str] = {}
        result[".github/routing-map.json"] = json.dumps(
            {"_base_autoloaded": {"note": "auto-generated by AgentPilot Orchestrator"}, **routing_map},
            indent=2, ensure_ascii=True,
        )
    ...
    def _gen_copilot_instructions(
        ...
        return (
            ...
            f"## DISPATCHER\n\n"
            f"### Session bootstrap (mandatory)\n"
            f"1. Run `python .github/router.py --stats` at session start.\n"
            f"2. Publish status header with router data.\n\n"
            f"### Per-request protocol (mandatory)\n"
            f"Before every operational response:\n"
            f"1. Route the request:\n"
            f"   ```\n"
            f"   python .github/router.py --direct \"<query>\"\n"
            f"   ```\n"
            f"2. Determine **token budget** from the `priority` field of the routing result:\n\n"
            f"   | Priority | Estimated budget | Typical use |\n"
            f"   |----------|-----------------|-------------|\n"
            f"   | `high`   | ~20 000 tok     | Critical fixes, core code |\n"
            f"   | `medium` | ~35 000 tok     | New features, refactoring |\n"
            f"   | `low`    | ~15 000 tok     | Docs, style, config |\n"
            f"   | no match | ~10 000 tok     | Quick question, clarification |\n\n"
            f"3. Check update status (manual-only policy):\n"
            f"   ```\n"
            ...
            f"   ```\n\n"
            f"4. Check local MCP server status:\n"
            f"   ```\n"
            f"   python .github/mcp_status.py\n"
            f"   ```\n\n"
            f"5. Publish mandatory header:\n"
            f"   ```\n"
            f"   🤖 <model> | Agent: <agent> | Scenario: <scenario> | Budget: ~<N>k tok | Routing: <stats> | Update: <ok-or-procedure> | MCP: <Active-or-Inactive>\n"
            f"   ```\n"
            f"   MCP rule:\n"
            f"   - if active: `MCP: Active`\n"
            f"   - if inactive: `MCP: Inactive (see .github/MCP_ACTIVATION.md)`\n"
            f"   Example if up-to-date:\n"
            f"   ```\n"
            f"   🤖 GPT-5.3-Codex | Agent: orchestratore | Scenario: _fallback | Budget: ~15k tok | Routing: 13scn/176kw|overlap:2.3%|[WARN] | Update: ok | MCP: Active\n"
            f"   ```\n"
            f"   Example if MCP is inactive:\n"
            f"   ```\n"
            f"   🤖 GPT-5.3-Codex | Agent: orchestratore | Scenario: _fallback | Budget: ~15k tok | Routing: 13scn/176kw|overlap:2.3%|[WARN] | Update: ok | MCP: Inactive (see .github/MCP_ACTIVATION.md)\n"
            f"   ```\n"
            f"   Example if update is available:\n"
            f"   ```\n"
            f"   🤖 GPT-5.3-Codex | Agent: orchestratore | Scenario: _fallback | Budget: ~15k tok | Routing: 13scn/176kw|overlap:2.3%|[WARN] | Update: git pull --ff-only | MCP: Inactive\n"
            f"   ```\n"
            f"6. Stay within the declared budget. If you expect to exceed it, warn the user and ask for confirmation.\n\n"
            f"### Agents\n| Agent | Domain |\n|-------|--------|\n{agent_table}\n\n"
            f"### Key scenarios\n{scenario_list}\n\n"
            f"### Router commands\n"
            f"```\n"
            ...
            f"python .github/mcp_status.py\n"
            f"python -c \"import json, sys; from pathlib import Path; sys.path.insert(0, str(Path('.github').resolve())); import update_manager; print(json.dumps(update_manager.get_update_status(refresh=True), indent=2, ensure_ascii=False))\"\n"
            f"```\n\n"
            f"### Repo exploration policy\n"
            f"1. Start from files routed by the router\n"
            f"2. Keep scope narrow if confidence > threshold\n"
            f"3. Expand to full repo only if `repo_exploration.allowed: true`\n\n"
