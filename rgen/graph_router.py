...
class GraphRouter:
    ...
    def _calculate_depth(self, scenario: str, visited: set[str]) -> int:
        ...
        max_dep_depth = 0
        for dep_scenario in deps:
            dep_depth = self._calculate_depth(dep_scenario, visited.copy())
    ...
    def route_with_graph(self, query: str, context: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        for secondary_agent in plan[1:]:  # Skip primary agent
            try:
                secondary_scenario = agent_to_scenario.get(secondary_agent)
                scoped_context = {
                    **forwarded_context,
                    "target_agent": secondary_agent,
                    "target_scenario": secondary_scenario,
                }
                secondary = self.route_query_fn(query, context=scoped_context)
                secondary_results.append(secondary)
                # Update context for next agent
                forwarded_context["prior_agent"] = secondary.get("agent", secondary_agent)
                forwarded_context["prior_confidence"] = secondary.get("confidence", 0.0)
                forwarded_context["prior_context"] = secondary.get("context", "")
                forwarded_context["prior_files"] = secondary.get("files", [])
                forwarded_context["prior_scenario"] = secondary.get("scenario", secondary_scenario)
